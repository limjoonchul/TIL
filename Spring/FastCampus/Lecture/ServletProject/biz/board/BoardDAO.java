...
public class BoardDAO {
    ...
    private static final String BOARD_TITLE = "SELECT * FROM BOARD WHERE TITLE LIKE %?% ORDER BY SEQ DESC";
    private static final String BOARD_CONTENT = "SELECT * FROM BOARD WHERE CONTENT LIKE %?% ORDER BY SEQ DESC";
...
//				System.out.println(rs.getInt("SEQ") + " : " + rs.getString("CONTENT"));
    ...
    public void titleSerchBoardList(BoardVO vo){
        // 비어있는 리스트 컬렉션을 생성한다.
        List<BoardVO> boardList = new ArrayList<BoardVO>();
        try {
            conn = JDBCUtil.getConnection();
            stmt = conn.prepareStatement(BOARD_TITLE);
